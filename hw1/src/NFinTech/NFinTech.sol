...
contract NFinTech is IERC721 {
    ...
    function setApprovalForAll(address operator, bool approved) external {
        require(operator != address(0), "Cannot approve the zero address");
        require(msg.sender != operator, "Cannot approve self as operator");
    ...
    function transferFrom(address from, address to, uint256 tokenId) public {
        require(_owner[tokenId] == from, "Not the owner of the token");
        require(to != address(0), "Transfer to zero address");
        require(msg.sender == from || msg.sender == _tokenApproval[tokenId] || isApprovedForAll(from, msg.sender), "Not authorized");
    ...
    function _transferLogic(address from, address to, uint256 tokenId) private {
        require(_owner[tokenId] == from, "Not the owner of the token");
        require(to != address(0), "Transfer to zero address");
        require(msg.sender == from || msg.sender == _tokenApproval[tokenId] || isApprovedForAll(from, msg.sender), "Not authorized");
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes calldata data) public {
        _transferLogic(from, to, tokenId);
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId) public {
        ...
        require(_checkOnERC721Received(from, to, tokenId, emptyData), "Transfer to non ERC721Receiver implementer");
    }
